...
class Net(object):
    ...
    def kids(self, image_tensors, length, target):
        ...
        save_image_normalized(tensor=generated, save_image_normalized=dest, nrow=generated.size(0))
        print_timestamp("Saved test result to " + dest)
        return dest
    ...
    def test_single(self, image_tensor, age, gender, target, watermark):
        ...
        for index_ in range(0, 6): # Add esse trecho de codigo para gravar as imagens separadas durante o aging_game. O original apenas grava o output consolidado
            #print(index_)
            #print(generated[index_,:,:,:].shape)
            new_name = os.path.basename(target).split('.')[0] + '_group_' + str(index_) + '.jpg'
            #print(new_name)
            save_image_normalized(tensor=generated[index_,:,:,:], fp=os.path.join(os.path.split(target)[0], new_name), nrow=generated[index_,:,:,:].size(0))
            print_timestamp("Saved image individual result to " + new_name)
    ...
    def teach(
        ...
        print('len dataset ImageFold: ', len(dataset))
        ...
        valid_dataset, train_dataset = torch.utils.data.random_split(dataset, (valid_size, train_size))
    ...
    def save(self, path, to_save_models=True):
        ...
        if to_save_models:
            if not os.path.isdir(path):
                os.mkdir(path)
                print('save models: criou pasta de epoca: ', path)
